...
def get_ikb_hero_data(url_name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(row_width=2).add(
        ...
        InlineKeyboardButton(text='К списку героев', callback_data=hero_back_to_heroes_cb_data.new(url_name=url_name))
    )
    ...
    if skill_num < max_skills:
        row.append(InlineKeyboardButton(text=f"{skill_num + 1} навык (след)",
                                        callback_data=hero_skills_pagination_cb_data.new(url_name=url_name,
                                                                                         skill_num=skill_num + 1)))
    ikb.add(*row)
    ikb.add(InlineKeyboardButton(text="Назад",
                                 callback_data=hero_skills_pagination_cb_data.new(url_name=url_name,
                                                                                  skill_num=0)))
...
def get_ikb_talents(url_name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup().add(
        InlineKeyboardButton(text="Назад", callback_data=hero_skills_pagination_cb_data.new(url_name=url_name,
                                                                                            skill_num=0))
    )

from config import PICKS_PER_DAY
...
async def daily_picks_distribution(dp):
    ...
    for user in users:
        await db_users.update_user_max_picks_per_day(user=user, num=PICKS_PER_DAY)
    await log_to_admins(dp, f"Ежедневная выдача всем пользователям бота {PICKS_PER_DAY} пиков выполнена!")